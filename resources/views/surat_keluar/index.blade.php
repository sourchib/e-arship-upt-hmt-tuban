...
<div class="container-fluid px-0">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            ...
            <p class="text-muted small mb-0">Kelola surat keluar dan disposisi</p>
        </div>
        <button type="button" class="btn" id="openCreateModal" style="background-color: var(--primary-green); color: white; border-radius: 8px; padding: 10px 20px; font-weight: 500;">
            <i data-lucide="plus" style="width: 18px; margin-right: 5px;"></i> Buat Surat Keluar
        </button>
    </div>
    ...
    <div class="bg-white p-4 rounded-2xl shadow-sm mb-4 d-flex flex-wrap gap-3 align-items-center justify-content-between">
        <div class="flex-grow-1" style="max-width: 800px;">
            <div class="relative">
                ...
                <input type="text" class="block w-full pl-10 pr-3 py-2.5 border border-slate-200 rounded-xl text-sm placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-green-500/20 focus:border-green-500 transition-all" placeholder="Cari nomor, perihal, atau tujuan...">
            </div>
        </div>
...
<!-- Modal Create Surat Keluar -->
<div class="modal-backdrop-custom" id="modalBackdrop"></div>
<div class="modal-container-custom" id="createSuratModal">
    <div class="modal-content-custom">
        <div class="modal-header-custom">
            <div>
                <h4 class="fw-bold mb-0">Tambah Surat Keluar</h4>
                <p class="text-muted small mb-0">Masukkan detail surat keluar baru</p>
            </div>
        ...
        <div class="modal-body-custom">
            <form action="{{ route('surat-keluar.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control @error('nomor_surat') is-invalid @enderror" id="nomor_surat" name="nomor_surat" value="{{ old('nomor_surat') }}" placeholder="Contoh: 001/SK/UPT-PTHMT/II/2026" required>
                    @error('nomor_surat')
                ...
                <div class="row mb-3">
                    <div class="col-md-6">
                        ...
                        <input type="text" class="form-control @error('tujuan') is-invalid @enderror" id="tujuan" name="tujuan" value="{{ old('tujuan') }}" placeholder="Nama tujuan/penerima" required>
                        @error('tujuan')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <!-- Default fields for backend compatibility -->
...
<script>
    ...
    // AJAX Searching
    const searchInput = document.querySelector('input[placeholder="Cari nomor, perihal, atau tujuan..."]');
    ...
    function bindSendConfirm() {
        document.querySelectorAll('.btn-send-confirm').forEach(button => {
            button.addEventListener('click', function() {
                const formId = this.getAttribute('data-form');
                Swal.fire({
                    title: 'Kirim Surat?',

...
@push('scripts')
    <script>
        ...
        // Table Header Sort Click Listener
        if (listContainer) {
            listContainer.addEventListener('click', function(e) {
                const th = e.target.closest('.sortable-col');
                if (th) {
                    const sortField = th.dataset.sort;
                    let newOrder = 'desc'; // Default to desc
                    
                    // Toggle if already sorting by this field
                    if (sortSelect && sortSelect.value.startsWith(sortField + '-')) {
                        const currentOrder = sortSelect.value.split('-')[1];

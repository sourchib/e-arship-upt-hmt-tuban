            ...
            <tbody>
                @forelse($suratMasuk as $index => $surat)
                    <tr>
                        ...
                        <td>
                            <span
                                style="color:#475569;font-size:13.5px;max-width:220px;display:block;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">{{ $surat->perihal }}</span>
                        </td>
                        <td>
                        ...
                        <td>
                            <span style="color:#64748b;font-size:13px;max-width:200px;display:block;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;" title="{{ $surat->disposisi }}">
                                {{ $surat->disposisi ?? '-' }}
                            </span>
                        </td>
                        ...
                        <td>
                            <div class="action-btns">
                                <button type="button" class="action-btn action-btn-view btn-view-detail"
                                    ...
                                    data-pengirim="{{ $surat->pengirim }}" data-penerima="{{ $surat->penerima }}"
                                    ...
                                    data-kategori="{{ $surat->kategori }}" data-file-path="{{ $surat->file_path }}"
                                    data-status="{{ $surat->status }}" data-status-class="{{ $sc }}" title="Detail">
                                    <i data-lucide="eye"></i>
                                </button>
                                ...
                                @if(Auth::check() && Auth::user()->role === 'Admin')
                                    <button type="button" class="action-btn action-btn-edit btn-edit-surat"
                                        data-id="{{ $surat->id }}" data-nomor="{{ $surat->nomor_surat }}"
                                        data-perihal="{{ $surat->perihal }}" data-pengirim="{{ $surat->pengirim }}"
                                        data-penerima="{{ $surat->penerima }}" data-disposisi="{{ $surat->disposisi }}"
                                        data-penerima-disposisi="{{ $surat->penerima_disposisi }}"
                                        data-tanggal="{{ $surat->tanggal_surat->format('Y-m-d') }}"
                                        data-tanggal-terima="{{ $surat->tanggal_terima->format('Y-m-d') }}"
                                        data-kategori="{{ $surat->kategori }}" data-prioritas="{{ $surat->prioritas }}"
                                        data-status="{{ $surat->status }}"
                                        data-url="{{ route('surat-masuk.update', $surat->id) }}" title="Edit">
    ...
    @forelse($suratMasuk as $index => $surat)
        <div class="mobile-card">
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:10px;">
                ...
                <div>
                    <div class="mobile-card-label">Penerima</div>
                    <div class="mobile-card-value" style="font-size:12px;">{{ $surat->penerima ?? '-' }}</div>
                </div>
                <div>
                ...
                <div>
                    ...
                    <div class="mobile-card-value" style="font-size:12px;">{{ $surat->tanggal_terima->format('d/m/Y') }}
                    </div>
                </div>
                <div>
                    <div class="mobile-card-label">Penerima Disposisi</div>
                    <div class="mobile-card-value" style="font-size:12px;">{{ $surat->penerima_disposisi ?? '-' }}</div>
                </div>
            </div>
            <div class="mobile-card-field" style="margin-bottom:10px;">
                <div class="mobile-card-label">Isi Disposisi</div>
                <div class="mobile-card-value" style="font-size:12px;">{{ $surat->disposisi ?? '-' }}</div>
            </div>
            <div class="mobile-card-footer">
                <div class="action-btns">
                    <button type="button" class="action-btn action-btn-view btn-view-detail"
                        data-nomor="{{ $surat->nomor_surat }}" data-perihal="{{ $surat->perihal }}"
                        data-pengirim="{{ $surat->pengirim }}" data-penerima="{{ $surat->penerima }}"
                        data-disposisi="{{ $surat->disposisi }}" data-penerima-disposisi="{{ $surat->penerima_disposisi }}"
                        data-tanggal-surat="{{ $surat->tanggal_surat->format('d/m/Y') }}"
                        data-tanggal-terima="{{ $surat->tanggal_terima->format('d/m/Y') }}"
                        data-kategori="{{ $surat->kategori }}" data-file-path="{{ $surat->file_path }}"
                        data-status="{{ $surat->status }}" data-status-class="{{ $sc }}" title="Detail">
                    ...
                    @if(Auth::check() && Auth::user()->role === 'Admin')
                        <button type="button" class="action-btn action-btn-edit btn-edit-surat" data-id="{{ $surat->id }}"
                            data-nomor="{{ $surat->nomor_surat }}" data-perihal="{{ $surat->perihal }}"
                            data-pengirim="{{ $surat->pengirim }}" data-penerima="{{ $surat->penerima }}"
                            data-disposisi="{{ $surat->disposisi }}" data-penerima-disposisi="{{ $surat->penerima_disposisi }}"
                            data-tanggal="{{ $surat->tanggal_surat->format('Y-m-d') }}"
                            data-tanggal-terima="{{ $surat->tanggal_terima->format('Y-m-d') }}"
                            data-kategori="{{ $surat->kategori }}" data-prioritas="{{ $surat->prioritas }}"
                            data-status="{{ $surat->status }}" data-url="{{ route('surat-masuk.update', $surat->id) }}"
                            title="Edit">
                            <i data-lucide="edit-3"></i>
                        </button>
                    @endif
                ...
                @if(Auth::check() && Auth::user()->role === 'Admin')
                    <button type="button" class="action-btn action-btn-delete btn-delete-confirm"
                        data-form="delete-form-{{ $surat->id }}">
                        <i data-lucide="trash-2"></i>
                    </button>
                @endif
            </div>
